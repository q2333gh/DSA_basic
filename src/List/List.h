...
public:
  ...
  auto selectMax(ListNodePosi<T> p,
                 int n) { //从p后的n个找最大.范围最大的元素的地址
    auto max = p;
    for (auto cur = p; 1 < n; n--)
      if (((cur = cur->succ)->data) >=
          max->data) // lt less than, !lt not less than : >=
        max = cur;
    return max;
  }
  ...
  void selectionSort() { selectionSort(*this, _size); }
};
